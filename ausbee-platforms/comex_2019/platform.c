...
void platform_init(void)
{
  ...
  mcual_gpio_clear(MCUAL_GPIOD, MCUAL_GPIO_PIN8 | MCUAL_GPIO_PIN9 | MCUAL_GPIO_PIN10);
  ...
  //Init experience pin (servo and coconium)
  mcual_gpio_init(MCUAL_GPIOD, MCUAL_GPIO_PIN11 | MCUAL_GPIO_PIN12 | MCUAL_GPIO_PIN13, MCUAL_GPIO_OUTPUT);
  mcual_gpio_clear(MCUAL_GPIOD, MCUAL_GPIO_PIN11);
  //Servos
  mcual_gpio_set_function(MCUAL_GPIOD, MCUAL_GPIO_PIN12, 2);
  mcual_gpio_set_function(MCUAL_GPIOD, MCUAL_GPIO_PIN13, 2);
  mcual_timer_init_servo(PLATFORM_TIMER_SERVO);
  
  //Reset in the middle position
  mcual_timer_set_duty_cycle(PLATFORM_TIMER_SERVO, MCUAL_TIMER_CHANNEL1, 1500);
  mcual_timer_set_duty_cycle(PLATFORM_TIMER_SERVO, MCUAL_TIMER_CHANNEL2, 1500);
  mcual_timer_enable_channel(PLATFORM_TIMER_SERVO, MCUAL_TIMER_CHANNEL1);
  mcual_timer_enable_channel(PLATFORM_TIMER_SERVO, MCUAL_TIMER_CHANNEL2);
...
void platform_gpio_set(uint32_t gpio)
{
  ...
  if(gpio & PLATFORM_GPIO_START_COCONIUM)
  {
...
uint32_t platform_gpio_get(uint32_t gpio)
{
  ...
  if(gpio & PLATFORM_GPIO_START_COCONIUM)
  {
    value |= mcual_gpio_get(MCUAL_GPIOD, MCUAL_GPIO_PIN11);
  }

  return value;
}

void platform_servo_set_value(uint32_t servo, uint32_t pos)
{
    if(pos < 550)
    {
        pos = 550;
    ...
    if(servo & PLATFORM_SERVO_0)
    {
        mcual_timer_set_duty_cycle(PLATFORM_TIMER_SERVO, MCUAL_TIMER_CHANNEL1, pos);
    }
    if(servo & PLATFORM_SERVO_1)
    {
        mcual_timer_set_duty_cycle(PLATFORM_TIMER_SERVO, MCUAL_TIMER_CHANNEL2, pos);
    }
}
