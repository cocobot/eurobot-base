...
GLOBAL_TEMPLATE_FILENAME = os.path.join(os.path.dirname(__file__), 'global_file_template.tmpl')
MOD_TEMPLATE_FILENAME = os.path.join(os.path.dirname(__file__), 'mod_file_template.tmpl')
...
def insert(root, mods, code):
    if len(mods) > 1:
        mod = mods[0]
        if not mod in root:
            root[mod] = {}
            root[mod]["#data"] = []
        insert(root[mod], mods[1:], code)
    else:
    ...
    if isinstance(root, str):
        ...
        for k in root:
            if k != "#data":
                ...
                t = Object()
                t.name = k
                t.data = fill_code(root[k])
                mod_template_expander = make_template_expander(MOD_TEMPLATE_FILENAME)
                r += generate(mod_template_expander, t)
        if "#data" in root:
            for k in root["#data"]:
                r += k
        return r
...
def run_generator(types, dest_dir, header_only):
    try:
        global_template_expander = make_template_expander(GLOBAL_TEMPLATE_FILENAME)
        ...
        write_generated_data(dest_dir, code, first)
    except Exception as ex:
        logger.info('Generator failure', exc_info=True)
        die(ex)

def write_generated_data(filename, data, first):
...
def type_to_rust_type(t):
    if t.category == t.CATEGORY_PRIMITIVE:
        ...
        return {
                ...
                'rust_type_comment':'%s %dbit[%d] max items' % (mode, values['bitlen'], t.max_size, ),
                'bitlen':values['bitlen'],
                'max_array_elements': t.max_size,
                'array_max_size_bit_len':t.max_size.bit_length(),
                'array_max_size_bit_len_expanded':expand_to_next_full(t.max_size.bit_length()),
                'rust_type_category': t.value_type.category,
                }
...
def generate_one_type(template_expander, t):
    ...
    t.split_name = t.full_name.split(".")
    t.struct_name = t.split_name[-1]
    ...
    # Attribute types
    def inject_rust_types(attributes):
    ...
    if t.kind == t.KIND_MESSAGE:
        t.has_array = inject_rust_types(t.fields)
        t.has_float16 = has_float16(t.fields)
        inject_rust_types(t.constants)
        t.all_attributes = t.fields + t.constants
        t.union = t.union and len(t.fields)
        if t.union:
            t.union = len(t.fields).bit_length()
    else:
        t.request_has_array = inject_rust_types(t.request_fields)
        t.request_has_float16 = has_float16(t.request_fields)
        inject_rust_types(t.request_constants)
        t.response_has_array = inject_rust_types(t.response_fields)
        t.response_has_float16 = has_float16(t.response_fields)
        inject_rust_types(t.response_constants)
...
def generate(template_expander, t):
    # Generation
    text = template_expander(t=t)  # t for Type
    text = '\n'.join(x.rstrip() for x in text.splitlines())
    text = text.replace('\n\n\n\n\n', '\n\n').replace('\n\n\n\n', '\n\n').replace('\n\n\n', '\n\n')
