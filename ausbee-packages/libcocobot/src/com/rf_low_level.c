...
static com_rf_packet_t _status_pkt =
{
  .type = COM_RF_PACKET_STATUS,
};

static void cocobot_com_rf_irq(void)
{
...
static void cocobot_com_rf_irq_task(void * unused)
{
  ...
  for( ;; )
  {
    ...
    if(xIrqStatus.IRQ_RX_DATA_READY)
    {
      uprintf("R\r\n");
      uint8_t buf[96];
      uint8_t src_id = SpiritPktCommonGetReceivedSourceAddress(); 
      int8_t rssi = SpiritQiGetRssidBm();
      int cRxData = SpiritLinearFifoReadNumElementsRxFifo();
      SpiritSpiReadLinearFifo(cRxData, buf);
      SpiritCmdStrobeRx();

      for(int i = 0; i < cRxData; i += sizeof(com_rf_packet_t))
      {
        com_rf_packet_t p;
        memcpy(&p, buf + i, sizeof(p));
        cocobot_com_rf_recv(src_id, rssi, &p);
      }
    }
...
void cocobot_com_rf_low_level_begin_tx(void)
{
  ...
  SpiritPktStackSetPayloadLength(sizeof(_status_pkt));
  SpiritPktStackSetDestinationAddress(BROADCAST_ADDR);

  /*
  SpiritCmdStrobeSabort();
  SpiritCmdStrobeFlushTxFifo();
  SpiritSpiWriteLinearFifo(sizeof(_status_pkt), (uint8_t *)(&_status_pkt));
...
void cocobot_com_rf_low_level_init(void)
{
  ...
  PktStackAddressesInit addressInit = 
  {
    ...
    cocobot_com_get_node_id(),         // my address (address of the current node)
    S_ENABLE,                          // enable/disable filtering on multicast address
    0xE1,                              // multicast address
  ...
  */
  ...
  {
    S_ENABLE,                          // enable/disable the autoack feature
